...
![Language](https://img.shields.io/badge/C%2B%2B-20-00599C?style=flat-square)
![Rust](https://img.shields.io/badge/Rust-stable-b7410e?style=flat-square)
![UI](https://img.shields.io/badge/UI-SolidJS-2c4f7c?style=flat-square)
![Runtime](https://img.shields.io/badge/Node.js-22%2B-3c873a?style=flat-square)
...
Fubuki Browser Alpha は、macOS 向けブラウザ基盤の MVP 実装です。ブラウザとしての完成度よりも、まずは **state ownership**、**protocol boundary**、**native host integration** を安定させることを重視しています。

| Item | Status |
|---|---|
| Target platform | macOS。Apple Silicon を主なターゲットにしています。 |
| Runtime model | CEF を直接利用し、Electron / Tauri / WKWebView には依存しません。 |
| State model | Rust 製 FrostEngine がタブ、ウィンドウ、設定、セッションを所有します。 |
| UI | SolidJS 製 UI を `fubuki://app/` から読み込みます。 |
| Release readiness | プロダクション用途ではありません。コード署名、ノータリゼーション、アップデーターは未提供です。 |
...
この分離により、UI がブラウザ状態の source of truth になることを避け、ネイティブホストが論理状態を抱え込むことも避けます。CEF は描画と Web 実行基盤として扱い、ブラウザとしての意思決定は FrostEngine 側に寄せます。
...
- ブラウザ基本操作の完成度向上
- 設定、履歴、ブックマーク、ダウンロード管理の UX 改善
- Frost Protocol の互換性管理
- 外部自動操作境界の安全性強化
- パッケージング、署名、配布導線の検討

このリストは実装計画の概要であり、リリース保証ではありません。
