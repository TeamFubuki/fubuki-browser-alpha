...
**C++20、CEF、Rust、SolidJS で構成する macOS 向けブラウザ基盤。**
...
- **ネイティブ前提** — macOS ネイティブホストが NSWindow と CEF Browser を管理します。
- **状態はエンジンが所有** — タブ、ウィンドウ、設定、セッションは FrostEngine が所有します。
- **明示的な通信境界** — UI と Host は Frost Protocol / HostCommand / HostEvent を通して接続します。
- **差し替え可能なレイヤー** — UI、Engine、Host を分離し、将来的な置き換えや別ホスト実装を妨げません。
- **自動操作を見据えた境界設計** — 外部操作は capability check と audit event を前提に、エンジンのコマンド層へ接続します。
- **重量級アプリラッパー不使用** — Electron、Tauri、WKWebView、WebView2 は使用しません。
