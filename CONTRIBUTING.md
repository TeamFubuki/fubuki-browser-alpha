...
Fubuki Browser Alpha は、macOS ネイティブホスト、CEF、Rust 製 FrostEngine、SolidJS UI を分離して構成するブラウザシェルです。

このプロジェクトで重要なのは、機能を足すことだけではありません。どのレイヤーが状態を持つのか、どの境界を通して通信するのか、どの操作が安全に実行できるのかを崩さないことです。
...
The heavier macOS native CEF build is intended for `main` or manual workflow runs, not every pull request.
...
This project contains browser-adjacent code. Treat boundaries as security-relevant even in Alpha.
