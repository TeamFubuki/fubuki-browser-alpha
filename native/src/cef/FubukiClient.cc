...
bool FubukiClient::OnPreKeyEvent(CefRefPtr<CefBrowser>, const CefKeyEvent& event, CefEventHandle,
  ...
  const bool handled = window_->HandleShortcut(
      commandDown, controlDown, shiftDown, altDown, event.windows_key_code,
      character);
...
bool FubukiClient::OnBeforeBrowse(CefRefPtr<CefBrowser>, CefRefPtr<CefFrame> frame,
  ...
  if (StartsWith(url, "fubuki://settings/set")) {
    ...
    // CEF does not consistently mark internal form submissions as a user
    // gesture. Trust POST only from our allow-listed internal pages; GET stays
    // non-mutating and destructive GET actions remain rejected below.
    if (method != "POST" && !user_gesture) {
      return true;
    }
