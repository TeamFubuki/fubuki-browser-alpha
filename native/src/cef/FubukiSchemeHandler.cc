...
std::string PageChrome(const std::string &title, const std::string &body) {
  ...
  html << "<!doctype html><html lang=\"" << HtmlEscape(lang) << "\" data-appearance=\"" << HtmlEscape(appearance) << "\"><head><meta charset=\"utf-8\"><title>"
...
html[data-appearance=dark] body{--bg:#14161a;--surface:#1d2025;--surface-2:#252932;--text:#f4f6f8;--muted:#a7b0bd;--line:rgb(255 255 255/.12);--hover:rgb(255 255 255/.07);--active:rgb(111 168 255/.14);--accent:#76a9ff;--danger:#ff8a80;--shadow:none;color-scheme:dark}
       ...
       << FubukiLogoSvg() << "<h1>" << HtmlEscape(Label(title)) << "</h1></header>" << body << "</main></body></html>";
  return html.str();
}
...
std::string DownloadsHtml() {
  ...
  body << "<div class=\"segmented\" style=\"margin-bottom:12px\"><a class=\"chip danger\" href=\""
       << ActionLink("clearData", "downloads", "fubuki://downloads/") << "\">" << Label("Clear downloads") << "</a></div>";
  if (records.empty()) {
    ...
    for (const auto& record : records) {
      body << "<article class=\"row\"><span aria-hidden=\"true\">↓</span><div><div class=\"title\">"
           ...
           << "<a class=\"chip\" href=\"" << ActionLink("openDownload", record.path, "fubuki://downloads/") << "\">" << Label("Open") << "</a>"
           << "<a class=\"chip\" href=\"" << ActionLink("revealDownload", record.path, "fubuki://downloads/") << "\">" << Label("Reveal") << "</a>"
           << "<a class=\"chip danger\" href=\"" << ActionLink("removeDownload", record.path, "fubuki://downloads/") << "\">" << Label("Remove") << "</a>"
...
std::string HistoryHtml() {
  ...
  body << "<form class=\"inline-form\" style=\"margin-bottom:12px\"><input type=\"search\" id=\"historySearch\" placeholder=\"" << Label("Search history") << "\" oninput=\"for(const row of document.querySelectorAll('[data-history-row]')) row.style.display=row.textContent.toLowerCase().includes(this.value.toLowerCase())?'grid':'none'\"></form>";
  body << "<div class=\"segmented\" style=\"margin-bottom:12px\"><a class=\"chip danger\" href=\""
       << ActionLink("clearHistoryRange", "lastHour", "fubuki://history/") << "\">" << Label("Clear last hour") << "</a><a class=\"chip danger\" href=\""
       << ActionLink("clearHistoryRange", "today", "fubuki://history/") << "\">" << Label("Clear today") << "</a><a class=\"chip danger\" href=\""
       << ActionLink("clearHistoryRange", "all", "fubuki://history/") << "\">" << Label("Clear all") << "</a></div>";
  if (records.empty()) {
    ...
    for (const auto& record : records) {
      const std::string day = record.createdAt.size() >= 10 ? record.createdAt.substr(0, 10) : Label("Earlier");
      if (day != currentDate) {
        currentDate = day;
      ...
      body << "<div data-history-row class=\"row\"><span class=\"favicon\"></span><a href=\"" << HtmlEscape(record.url) << "\" title=\"" << HtmlEscape(record.url)
           ...
           << "</span></a><a class=\"button danger\" href=\"" << ActionLink("removeHistory", record.url, "fubuki://history/") << "\">" << Label("Delete") << "</a></div>";
    }
    body << "</div>";
  }
...
std::string SettingsHtml() {
  ...
  body << "<div class=\"settings-layout\">"
       ...
       << "<a href=\"#general\">" << Label("General") << "</a><a href=\"#appearance\">" << Label("Appearance") << "</a><a href=\"#language\">" << Label("Language") << "</a><a href=\"#tabs\">" << Label("Tabs") << "</a>"
       << "<a href=\"#windows\">" << Label("Windows") << "</a><a href=\"#search\">" << Label("Search") << "</a><a href=\"#privacy\">" << Label("Privacy") << "</a>"
       << "<a href=\"#downloads\">" << Label("Downloads section") << "</a><a href=\"#developer\">" << Label("Developer") << "</a>"
       << "</nav><section class=\"settings-content\">"
       << "<form class=\"inline-form settings-search\"><input type=\"search\" placeholder=\"" << Label("Search settings") << "\" oninput=\"for(const field of document.querySelectorAll('[data-setting-section]')) field.style.display=field.textContent.toLowerCase().includes(this.value.toLowerCase())?'grid':'none'\"></form>"
       << "<div id=\"general\" class=\"field\" data-setting-section><span>" << Label("General") << "</span><div class=\"section-kicker\">Choose how Fubuki starts and where Home opens.</div><div class=\"segmented\">"
       ...
       << HtmlEscape(Setting("homeUrl", "https://example.com")) << "\" placeholder=\"" << Label("Home page URL") << "\"><button class=\"button\">" << Label("Save") << "</button><a class=\"button\" href=\""
       << ActionLink("resetSetting", "startupBehavior", "fubuki://settings/") << "\">" << Label("Reset") << "</a></form></div>"
       << "<div id=\"appearance\" class=\"field\" data-setting-section><span>" << Label("Appearance") << "</span><div class=\"section-kicker\">Use a flat system, light, or dark internal page theme.</div><div class=\"segmented\">"
       ...
       << "</div><a class=\"button\" href=\"" << ActionLink("resetSetting", "appearance", "fubuki://settings/") << "\">" << Label("Reset") << "</a></div>"
       << "<div id=\"language\" class=\"field\" data-setting-section><span>" << Label("Language") << "</span><div class=\"section-kicker\">Choose UI language. System follows your macOS language when possible.</div><div class=\"segmented\">"
       ...
       << "</div><a class=\"button\" href=\"" << ActionLink("resetSetting", "language", "fubuki://settings/") << "\">" << Label("Reset") << "</a></div>"
       << "<div id=\"tabs\" class=\"field\" data-setting-section><span>" << Label("Tabs") << "</span><div class=\"section-kicker\">Tune the new tab destination and page zoom default.</div><div class=\"segmented\">"
       ...
       << HtmlEscape(Setting("defaultZoomLevel", "0")) << "\" placeholder=\"" << Label("Default zoom level") << "\"><button class=\"button\">" << Label("Save") << "</button><a class=\"button\" href=\""
       << ActionLink("resetSetting", "defaultZoomLevel", "fubuki://settings/") << "\">" << Label("Reset") << "</a></form></div>"
       << "<div id=\"windows\" class=\"field\" data-setting-section><span>" << Label("Windows") << "</span><div class=\"section-kicker\">Control browser chrome visibility.</div><div class=\"segmented\">"
       << chip("sidebarVisible", sidebarVisible, "show", Label("Show sidebar"))
       << chip("sidebarVisible", sidebarVisible, "hide", Label("Hide sidebar"))
       << "</div><form class=\"inline-form\" action=\"fubuki://settings/set\" method=\"get\"><input type=\"hidden\" name=\"key\" value=\"sidebarWidth\"><input type=\"hidden\" name=\"return\" value=\"fubuki://settings/\"><input name=\"value\" value=\""
       << HtmlEscape(Setting("sidebarWidth", "196")) << "\" placeholder=\"" << Label("Sidebar width") << "\"><button class=\"button\">" << Label("Save") << "</button><a class=\"button\" href=\""
       << ActionLink("resetSetting", "sidebarWidth", "fubuki://settings/") << "\">" << Label("Reset sidebar width") << "</a></form></div>"
       << "<div id=\"search\" class=\"field\" data-setting-section><span>" << Label("Search") << "</span><div class=\"section-kicker\">Set the engine used from the omnibox and new tab page.</div><div class=\"segmented\">"
       ...
       << HtmlEscape(customSearchUrl) << "\" placeholder=\"https://example.com/search?q={query}\"><button class=\"button\">" << Label("Save") << "</button><a class=\"button\" href=\""
       << ActionLink("resetSetting", "searchEngine", "fubuki://settings/") << "\">" << Label("Reset") << "</a></form></div>"
       << "<div id=\"privacy\" class=\"field\" data-setting-section><span>" << Label("Privacy") << "</span><div class=\"section-kicker\">Clear local browsing records and web storage.</div><div class=\"segmented\">"
       << "<a class=\"chip danger\" href=\"" << ActionLink("clearData", "history", "fubuki://settings/") << "\">" << Label("History") << "</a>"
       ...
       << "<a class=\"chip danger\" href=\"" << ActionLink("clearData", "downloads", "fubuki://settings/") << "\">" << Label("Downloads") << "</a>"
       << "<a class=\"chip danger\" href=\"" << ActionLink("clearData", "all", "fubuki://settings/") << "\">" << Label("Clear all") << "</a>"
       << "</div></div>"
       << "<div id=\"downloads\" class=\"field\" data-setting-section><span>" << Label("Downloads section") << "</span><div class=\"section-kicker\">Set download confirmation and the default folder.</div><div class=\"segmented\">"
       ...
       << HtmlEscape(Setting("downloadDirectory", "")) << "\" placeholder=\"" << Label("Download directory") << "\"><button class=\"button\">" << Label("Save") << "</button><a class=\"button\" href=\""
       << ActionLink("resetSetting", "downloadDirectory", "fubuki://settings/") << "\">" << Label("Reset") << "</a></form></div>"
       ...
       << "<div id=\"developer\" class=\"field\" data-setting-section><span>" << Label("Developer") << "</span><div class=\"section-kicker\">Inspect the app shell and internal diagnostics.</div><div class=\"segmented\"><a class=\"chip\" href=\""
       << ActionLink("openDevTools", "1", "fubuki://settings/") << "\">" << Label("Open DevTools") << "</a><a class=\"chip\" href=\"fubuki://debug/\">" << Label("Debug page") << "</a></div></div>"
       << "</section></div>";
...
std::string DebugHtml() {
  ...
  body << "<div class=\"field\"><span>" << Label("Profile path") << "</span><div class=\"meta\">" << HtmlEscape(ProfilePath().string()) << "</div></div>";
  if (app) {
    body << "<div class=\"field\"><span>" << Label("Windows and tabs") << "</span><div class=\"list\">";
    ...
    body << "<div class=\"field\"><span>" << Label("Registered commands") << "</span><div class=\"list\">";
    ...
    body << "<div class=\"field\"><span>" << Label("Recent events") << "</span><div class=\"list\">";
  ...
  body << "</div></div>";
  body << "<div class=\"field\"><span>" << Label("Actions") << "</span><div class=\"segmented\"><a class=\"chip\" href=\""
       << ActionLink("openDevTools", "1", "fubuki://debug/") << "\">" << Label("Open DevTools") << "</a></div></div>";
  body << "</section>";
...
std::string NewTabHtml() {
  ...
  const std::string lang = BrowserLanguage();
  std::ostringstream html;
  html << "<!doctype html><html lang=\"" << HtmlEscape(lang) << "\" data-appearance=\"" << HtmlEscape(appearance) << R"("><head><meta charset="utf-8"><title>)" << Label("New Tab") << R"(</title>)" << FubukiFaviconLink() << R"(<style>
*{box-sizing:border-box}html,body{height:100%}@keyframes pageIn{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}}@keyframes focusPulse{0%{box-shadow:0 0 0 0 rgb(31 111 235/.24)}100%{box-shadow:0 0 0 7px transparent}}body{margin:0;display:grid;place-items:center;background:#f5f6f8;color:#15171a;font:15px -apple-system,BlinkMacSystemFont,"SF Pro Text","Hiragino Sans","Hiragino Kaku Gothic ProN","Yu Gothic","Helvetica Neue",sans-serif;letter-spacing:0}html[data-appearance=dark] body{background:#14161a;color:#f4f6f8;color-scheme:dark}main{width:min(620px,calc(100vw - 40px));display:grid;gap:20px;justify-items:center;animation:pageIn .34s cubic-bezier(.2,.8,.2,1)}.logo{width:58px;height:58px}h1{margin:0;font-size:32px;line-height:1;font-weight:720}form{width:100%}input{width:100%;height:44px;border:1px solid rgb(24 32 44/.14);border-radius:7px;background:#fff;padding:0 13px;font:inherit;outline:0;transition:border-color .16s ease,background .16s ease}html[data-appearance=dark] input{background:#1d2025;border-color:rgb(255 255 255/.12);color:#f4f6f8}@media(prefers-color-scheme:dark){html[data-appearance=system] body{background:#14161a;color:#f4f6f8;color-scheme:dark}html[data-appearance=system] input{background:#1d2025;border-color:rgb(255 255 255/.12);color:#f4f6f8}}input:focus{border-color:#1f6feb;animation:focusPulse .5s ease}@media(prefers-reduced-motion:reduce){*,*::before,*::after{animation:none!important;transition:none!important}}html[data-appearance=dark] body{background:#14161a;color:#f4f6f8;color-scheme:dark}html[data-appearance=dark] input{background:#1d2025;border-color:rgb(255 255 255/.12);color:#f4f6f8}
       ...
       << FubukiLogoSvg() << R"(<h1>Fubuki Browser Alpha</h1><form action="fubuki://newtab/search" method="get"><input name="q" autofocus autocomplete="off" placeholder=")" << Label("Search or enter URL") << R"("></form></main></body></html>)";
  return html.str();
}
