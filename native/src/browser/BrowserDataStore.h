...
public:
  ...
  bool RemovePermission(const std::string &origin,
                        const std::string &permission);
  void AddDownload(const std::string &url, const std::string &path,
                   const std::string &state);
  void UpdateDownload(const std::string &url, const std::string &path,
...
private:
  ...
  void MigrateRecordsJson(const std::filesystem::path &path,
                          const std::string &table);
  void RefreshCaches();
  void RefreshSettings();
  void RefreshList(const std::string &table, CefRefPtr<CefListValue> target,
                   size_t limit);
