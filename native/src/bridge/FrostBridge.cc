...
void *OpenPersistentEngine(const std::string &databasePath) {
  if (databasePath.empty()) {
    // An empty profile path is an explicit request for a temporary engine;
    // never pass a null path to the persistent constructor, which rejects it.
    return frost_engine_new_in_memory();
  }
