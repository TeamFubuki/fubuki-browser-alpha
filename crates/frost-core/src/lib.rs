...
use frost_protocol::{
    ...
    Request, Response, SettingChanged, TabActivated, TabClosed, TabPatch, TabsReordered,
};
use frost_store::{
        ...
        + ClearRepository,
{
    pub fn with_adapter_and_settings(adapter: A, repository: S) -> Self {
        ...
        // The first host window has this stable id. Subsequent ids are
        // generated by WindowService and carried in HostCommand payloads.
        let window_id = windows.create_window_with_id("window-1".into(), false);
        ...
        match request {
            ...
            Request::TabsClose { tab_id } => {
                ...
                if closed {
                    ...
                    if let Some(window_id) = closing_tab.map(|tab| tab.window_id) {
                        self.ensure_window_has_tab(&window_id)?;
                        self.emit_active_tab(&window_id);
                    }
                }
            ...
            Request::UiSetOverlayActive {
                ...
                self.adapter
                    .set_ui_overlay(active, width, height)
                    .map_err(|e| CoreError::Message(e.to_string()))?;
                Ok(Response::Bool(true))
            }
    ...
    fn ensure_window_has_tab(&mut self, window_id: &str) -> CoreResult<()> {
        if !self.tabs.tabs_in_window(window_id).is_empty() {
            return Ok(());
        }
        let tab = self
            .tabs
            .create_tab(window_id.to_owned(), "fubuki://newtab/".into(), true);
        self.windows.attach_tab(window_id, &tab.id, true);
        self.adapter
            .create_page(&tab.id, window_id, &tab.url, tab.is_active)
            .map_err(|e| CoreError::Message(e.to_string()))?;
    ...
    fn emit_active_tab(&mut self, window_id: &str) {
    ...
    fn emit_tab_order(&mut self, window_id: &str) {
        let tab_ids = self
            .tabs
            .tabs_in_window(window_id)
            .into_iter()
            .map(|tab| tab.id)
            .collect();
        self.emit(Event::TabsReordered(TabsReordered {
            window_id: window_id.to_owned(),
            tab_ids,
        }));
    }
...
mod tests {
    ...
    #[test]
    fn closing_active_tab_selects_right_neighbor_and_never_leaves_window_empty() {
        let mut core = BrowserCore::new();
        for url in [
            "https://one.example",
            "https://two.example",
        ...
        let middle = state.tabs[1].id.clone();
        ...
        assert!(
            core.process(ProtocolRequest::new(Request::TabsClose { tab_id: middle }))
                .ok
        );
        ...
        assert_eq!(state.tabs[1].url, "https://three.example");
        assert!(state.tabs[1].is_active);

        for tab in state.tabs.clone() {
            assert!(
                core.process(ProtocolRequest::new(Request::TabsClose { tab_id: tab.id }))
                    .ok
            );
        }
        let snapshot = core.process(ProtocolRequest::new(Request::AppSnapshot));
    ...
    #[test]
    fn closing_many_tabs_emits_one_host_close_per_closed_browser_and_reorder_is_differential() {
        let (host_tx, host_rx) = crossbeam_channel::unbounded();
        ...
        assert!(matches!(
            ...
            Event::TabsReordered(_)
        ));

        let target = state.tabs[0].id.clone();
        core.process(ProtocolRequest::new(Request::TabsCloseOther {
            tab_id: target,
        }));
        let closes = std::iter::from_fn(|| host_rx.try_recv().ok())
    ...
    #[test]
    fn host_history_event_persists_and_emits_a_differential_update() {
        let mut core = BrowserCore::new();
        core.process_host_event(HostEventEnvelope::new(HostEvent::HistoryVisited {
            title: "Example".into(),
        ...
        assert!(matches!(
            core.recent_events().last().map(|event| &event.event),
            Some(Event::HistoryChanged { url: Some(url) }) if url == "https://example.com"
        ));
    }
