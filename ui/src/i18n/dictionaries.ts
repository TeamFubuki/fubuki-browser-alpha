export const dictionaries = {
  en: {
    'action.addBookmark': 'Add bookmark',
    'action.clear': 'Clear',
    'action.close': 'Close',
    'action.closeFind': 'Close find',
    'action.closeTab': 'Close tab',
    'action.open': 'Open',
    'action.remove': 'Remove',
    'action.removeBookmark': 'Remove bookmark',
    'action.reset': 'Reset',
    'action.revealInFinder': 'Reveal in Finder',
    'action.save': 'Save',
    'commandPalette.empty': 'No commands',
    'commandPalette.placeholder': 'Search commands',
    'commandPalette.title': 'Command Palette',
    'commandPalette.toggleQuietMode': 'Toggle Quiet Mode',
    'common.back': 'Back',
    'common.bookmarks': 'Bookmarks',
    'common.debug': 'Debug',
    'common.downloads': 'Downloads',
    'common.find': 'Find',
    'common.forward': 'Forward',
    'common.history': 'History',
    'common.home': 'Home',
    'common.newTab': 'New tab',
    'common.privateWindow': 'Private window',
    'common.reload': 'Reload',
    'common.searchOrEnterUrl': 'Search or enter URL',
    'common.settings': 'Settings',
    'common.stop': 'Stop',
    'common.tabs': 'Tabs',
    'common.toggleSidebar': 'Toggle sidebar',
    'find.next': 'Next match',
    'find.previous': 'Previous match',
    'language.en': 'English',
    'language.ja': 'Japanese',
    'language.system': 'System',
    'permission.allow': 'Allow',
    'permission.block': 'Block',
    'permission.camera': 'Camera',
    'permission.geolocation': 'Location',
    'permission.keyboardLock': 'Keyboard lock',
    'permission.microphone': 'Microphone',
    'permission.notifications': 'Notifications',
    'permission.pointerLock': 'Pointer lock',
    'permission.private': 'This decision applies only to this private window.',
    'permission.title': 'Permission request',
    'permission.notNow': 'Not now',
    'quietMode.label': 'Quiet Mode',
    'sidebar.resize': 'Resize sidebar',
    'tabs.pinned': 'Pinned tabs',
    'tabs.search': 'Search tabs',
    'tabs.open': 'Open tabs',
  },
  ja: {
    'action.addBookmark': 'ブックマークに追加',
    'action.clear': '消去',
    'action.close': '閉じる',
    'action.closeFind': 'ページ内検索を閉じる',
    'action.closeTab': 'タブを閉じる',
    'action.open': '開く',
    'action.remove': '削除',
    'action.removeBookmark': 'ブックマークを削除',
    'action.reset': 'リセット',
    'action.revealInFinder': 'Finderで表示',
    'action.save': '保存',
    'commandPalette.empty': '一致するコマンドはありません',
    'commandPalette.placeholder': 'コマンドを検索',
    'commandPalette.title': 'コマンドパレット',
    'commandPalette.toggleQuietMode': 'Quiet Modeを切り替え',
    'common.back': '戻る',
    'common.bookmarks': 'ブックマーク',
    'common.debug': 'デバッグ',
    'common.downloads': 'ダウンロード',
    'common.find': '検索',
    'common.forward': '進む',
    'common.history': '履歴',
    'common.home': 'ホーム',
    'common.newTab': '新しいタブ',
    'common.privateWindow': 'プライベートウィンドウ',
    'common.reload': '再読み込み',
    'common.searchOrEnterUrl': '検索語句またはURLを入力',
    'common.settings': '設定',
    'common.stop': '停止',
    'common.tabs': 'タブ',
    'common.toggleSidebar': 'サイドバーを切り替え',
    'find.next': '次の一致',
    'find.previous': '前の一致',
    'language.en': '英語',
    'language.ja': '日本語',
    'language.system': 'システム設定',
    'permission.allow': '許可',
    'permission.block': 'ブロック',
    'permission.camera': 'カメラ',
    'permission.geolocation': '位置情報',
    'permission.keyboardLock': 'キーボードロック',
    'permission.microphone': 'マイク',
    'permission.notifications': '通知',
    'permission.pointerLock': 'ポインターロック',
    'permission.private':
      'この決定はこのプライベートウィンドウ内だけに適用されます。',
    'permission.title': '権限のリクエスト',
    'permission.notNow': '今回は許可しない',
    'quietMode.label': 'Quiet Mode',
    'sidebar.resize': 'サイドバーの幅を変更',
    'tabs.pinned': '固定タブ',
    'tabs.search': 'タブを検索',
    'tabs.open': '開いているタブ',
  },
} as const;

export type Language = keyof typeof dictionaries;
export type I18nKey = keyof typeof dictionaries.en;
